"""
Knowledge Cartography (Tao) - Dialectical Knowledge Mapping
...
See: KNOWLEDGE_CARTOGRAPHY.md for full design documentation
"""
...
logger = logging.getLogger("tao.storage")
...
class KnowledgeCartography:
    ...
    def __init__(self, domain_path: str, config: Optional[Dict] = None):
        ...
        self.classify_questions = self.config.get("classify_questions", True)  # NEW: Auto-classify

        # File paths
        self.history_file = os.path.join(domain_path, "query_history.json.gz")

        # Question classifier (lazy loaded)
        self._classifier = None

        logger.info(f"Tao storage initialized for {domain_path} (enabled={self.enabled}, "
                   f"context_window={self.context_window}, classify={self.classify_questions})")

    @property
    def classifier(self):
        """Lazy-load question classifier."""
        if self._classifier is None and self.classify_questions:
            try:
                from tao.analysis.sophistication import QuestionClassifier
                self._classifier = QuestionClassifier()
                logger.debug("Question classifier loaded")
            except Exception as e:
                logger.warning(f"Failed to load question classifier: {e}")
    ...
    def save_query_response(
        ...
        Args:
            ...
            parent_query_id: Optional ID of the query that led to this one (for causality tracking)
        ...
        try:
            ...
            # Auto-detect parent query if not provided (simple heuristic)
            if parent_query_id is None and len(history) > 0:
                parent_query_id = self._detect_parent_query(history, query)

            # Classify question sophistication if enabled
            metadata = metadata or {}
            if self.classify_questions and self.classifier:
                try:
                    domain_name = os.path.basename(self.domain_path)
                    classification = self.classifier.classify_question(query, domain=domain_name)
                    metadata["question_level"] = classification["level"]
                    metadata["question_level_confidence"] = classification["confidence"]
                    metadata["question_level_reasoning"] = classification["reasoning"]
                    logger.debug(f"Classified question as L{classification['level']}: {query[:50]}...")
                except Exception as e:
            ...
            entry = {
                ...
                "response": response,
                "metadata": metadata,
                "parent_query_id": parent_query_id,  # None if first query or no parent detected
                "evoked_questions": []  # Initialize empty, populate below if needed
            }
    ...
    def _detect_parent_query(self, history: List[Dict], current_query: str) -> Optional[int]:
        """
        Auto-detect parent query using simple heuristics.

        Heuristic: Query within 2 minutes + keyword overlap = likely related

        Args:
            history: Current query history
            current_query: The query being saved

        Returns:
            Parent query ID if detected, None otherwise
        """
        if not history:
            return None
        ...
        try:
            ...
            # Get current time (use same timezone logic as save_query_response)
            tz_name = os.getenv("APP_TIMEZONE", "America/Vancouver")
            try:
                tz = zoneinfo.ZoneInfo(tz_name)
            ...
            time_gap = (current_time - last_timestamp).total_seconds() / 60

            # If within 2 minutes, check keyword overlap
            if time_gap <= 2.0:
                # Simple keyword overlap check (lowercase, split on spaces)
                current_keywords = set(current_query.lower().split())
                last_keywords = set(last_entry["query"].lower().split())

                # Remove common stop words for better matching
                stop_words = {"the", "a", "an", "and", "or", "but", "in", "on", "at", "to", "for",
                             "of", "is", "are", "was", "were", "what", "how", "why", "when", "where"}
                current_keywords -= stop_words
                last_keywords -= stop_words

                # If at least 2 keywords overlap, consider it related
                overlap = current_keywords & last_keywords
                if len(overlap) >= 2:
                    logger.debug(f"Auto-detected parent query #{last_entry['id']} "
                               f"(gap={time_gap:.1f}min, overlap={len(overlap)} keywords)")
                    return last_entry["id"]

        except Exception as e:
            logger.warning(f"Failed to auto-detect parent query: {e}")
