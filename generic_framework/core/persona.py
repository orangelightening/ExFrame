...
class Persona:
    ...
    async def respond(
        ...
        if override_patterns:
            ...
            # If Brave Search was used, return directly (skip LLM synthesis)
            if self.data_source == "internet" and content and content.startswith("Web search results (Brave Search):"):
                self.logger.info("⚡ Returning Brave Search results directly (data source path)")

                # Extract Brave answer from formatted content
                brave_answer = content.replace("Web search results (Brave Search):\n\n", "")

                # Clean up LaTeX formatting (Brave uses $ for math mode)
                import re
                # Remove $ symbols used for LaTeX math mode
                brave_answer = re.sub(r'\$([^\$]+)\$', r'\1', brave_answer)
                self.logger.info("Cleaned LaTeX formatting from Brave response")

                # Add sources section
                if not brave_answer.endswith('\n'):
                    brave_answer += '\n'
                brave_answer += '\n---\n\n'
                brave_answer += '**Sources:** This answer was generated by Brave Search AI, '
                brave_answer += 'which synthesizes information from current web sources. '
                    ...
                    if "tool_calls" in message and message["tool_calls"]:
                        ...
                        if function_name == "web_search":
                            ...
                            if brave_search_used:
                                ...
                                # Clean up LaTeX formatting (Brave uses $ for math mode)
                                import re
                                cleaned_content = re.sub(r'\$([^\$]+)\$', r'\1', search_content)
                                self.logger.info("Cleaned LaTeX formatting from Brave response")
